...
;-----------------------------------------------------------
	...
	*Sub_Input
		...
		[endscript]
			...
			; ★ JavaScriptについて補足①：ifとはなにか
			; 
			; 次のように書くと、『「条件」が正しければ「処理」を実行しろ』という意味になります。
			; 
			;     if (条件) {
			;         処理
			;     }
			
			; ★ JavaScriptについて補足②：文字列の特殊能力①
			;
			; 変数 f.input に入っているのは文字列です。
			; 実は文字列には隠された特殊な能力がありまして、それは何かというと、
			; 『変数名の後ろに「.length」を付けると、その変数に入っている文字列の「長さ（＝字数）」が得られる』
			; というものです。
			; つまり、
			; 
			;     f.input.length
...
;-----------------------------------------------------------
		...
		……。[p]
		とまあ、これが一例です。[p]
		これの何がよいかというと、開発者目線だと「入力内容を制限できる」というところが大きいかな。[p]
